...
mothur "#chimera.uchime(fasta=$fn.unique.precluster.fasta,reference=$MOTHUR_REF_DATA/silva.gold.align, chimealns=T, minh=0.3, mindiv=0.5, xn=8.0, dn=1.4, xa=1.0, chunks=4, minchunk=64, idsmoothwindow=32, maxp=2, skipgaps=T, skipgaps2=T, minlen=10, maxlen=10000, ucl=false, queryfract=0.5)"
...
mothur "#align.seqs(candidate=$fn.unique.precluster.pick.fasta, template=$MOTHUR_REF_DATA/silva.bacteria.fasta, search=kmer, ksize=8, align=needleman, match=1, mismatch=-2, gapopen=-1, flip=t, threshold=0.5, processors=$NSLOTS)"
...
mothur "#align.seqs(candidate=$fn.unique.precluster.pick.filter.phylip.nn.0.03.rep.fasta, template=$MOTHUR_REF_DATA/silva.bacteria.fasta, search=kmer, ksize=8, align=needleman, match=1, mismatch=-2, gapopen=-1, flip=t, threshold=0.5, processors=$NSLOTS)"
...
mothur "#classify.seqs(fasta=$fn.unique.precluster.pick.filter.phylip.nn.0.03.rep.fasta, template=$MOTHUR_REF_DATA/silva.bacteria.fasta, taxonomy=$MOTHUR_REF_DATA/silva.bacteria.silva.tax,method=bayesian,cutoff=95,probs=f)"
